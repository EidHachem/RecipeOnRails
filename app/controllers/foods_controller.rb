class FoodsController < ApplicationController
  ...
  def create
    @user = current_user
    @food = current_user.foods.new(user_id: current_user.id, name: params[:name], price: params[:price], quantity: params[:quantity], measurement_unit: params[:measurement_unit])
    if @food.save
